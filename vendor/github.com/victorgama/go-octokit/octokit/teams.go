...
var (
	...
	TeamsRepositoryURL   = Hyperlink("/repos/{owner}/{repo}/teams")
)
...
func (t *TeamsService) AddMembership(uri *Hyperlink, uriParams M, role string) (
	...
	if role == "" {
		result = t.client.put(url, nil, &membership)
	} else {
...
func (t *TeamsService) GetTeamsForRepository(uri *Hyperlink, uriParams M) (
	teams []Team, result *Result) {
